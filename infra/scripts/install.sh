...
if [ -x "$(command -v apt-get)" ]; then
 ...
 # Removed for debugging (time-wise) purposes
 # sudo yum update -y
 # sudo yum install -y unzip wget
 echo "Not updating due to speeding it up"
fi
...
CONSUL_FLAGS="-server -bootstrap-expect=${SERVER_COUNT} -join=${CONSUL_JOIN} -data-dir=/opt/consul/data -client=0.0.0.0"
EOF
